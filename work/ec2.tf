resource "aws_instance" "sample-ec2-01" {
  ami = "ami-040c33c6a51fd5d96"   #각자 생성한 ami
...
resource "aws_instance" "sample-ec2-03" {
  ...
  user_data = <<-EOF
    ...
    systemctl start nginx
    systemctl enable nginx
    echo "This is Test Server -------- 3" > /var/www/html/index.nginx-debian.html
  EOF
  root_block_device {
    volume_size = "30"
    volume_type = "gp3"
...
resource "aws_instance" "sample-ec2-04" {
  ami = "ami-040c33c6a51fd5d96"   #각자 생성한 ami
  instance_type = "t2.micro"
  key_name      = "kosta_edu"
  associate_public_ip_address = true # 퍼블릭 IP 자동 할당
  ...
  user_data = <<-EOF
    ...
    systemctl start nginx
    systemctl enable nginx
    echo "This is Test Server -------- 4" > /var/www/html/index.nginx-debian.html
  EOF
  root_block_device {
    volume_size = "30"
    volume_type = "gp3"
